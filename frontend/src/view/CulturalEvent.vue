<template>
    <div class ="free"> <!--일단 free board 복붙해놨음-->
        <span style="width: 150px; margin-left: 600px;">Cultural Events</span>
        ...
        <div v-if="viewMethod=='grid'" id="gridBoard">
            <div class ="board" v-for ="(board, index) in culturalEvents" :key ="board.title">
                <router-link :to = "{name : 'culturalEventDetail', params: {idx : index}}"><img v-if="board.url" v-bind:src="'images/' + board.url" style="height : 200px; width: 250px;">
        ...
        <div v-if="viewMethod=='lined'" id="linedBoard">
            <table style="width: 1230px; margin-left: 140px;">
                ...
                <th>Title</th>
                <br>
                <tr class ="linedBoard" v-for ="(board, index) in culturalEvents" :key ="board.title">
                    <td align="center" style ="color : #566270;">{{ board.writer }}</td>
                    <td align="center" style ="color : #566270;">{{ board.email }}</td>
                    <router-link :to = "{name : 'culturalEventDetail', params: {idx : index}}"><td align="left" style ="color : #566270; left: 640px">{{ board.title }}</td></router-link>
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                culturalEvents : culturalEventsExample,
